...
- `src/main/java/com/whatxe/xlib/command/AbilityGrantCommands.java`
  Root `/xlib` registration surface for admin, debug, and content-inspection trees. Wires the ability, passive, item, recipe, progression, profile, capability-policy, classification, binding, stage, visual-form, body-transition, and debug subtrees into the `xlib` command root.
...
- `src/main/java/com/whatxe/xlib/attachment/ModAttachments.java`
  ...
  - `player_tracked_values` via `TrackedValueData` (synced)
  - `entity_classifications` via `EntityClassificationData` (all entities, copied on death)
...
- `src/main/java/com/whatxe/xlib/combat/DamageModifierProfileDefinition.java`
- `src/main/java/com/whatxe/xlib/combat/DamageModifierProfileData.java`
...
- `src/main/java/com/whatxe/xlib/presentation/ProgressionTreeLayout.java`
...
- `src/main/java/com/whatxe/xlib/presentation/ProgressionLayoutPlanner.java`
  Built-in progression presentation registry and reusable layout planner for list mode plus icon-node tree skill-tree arrangements, including addon-authored tree spacing, label width, and label wrapping policy through `ProgressionTreeLayout`.
