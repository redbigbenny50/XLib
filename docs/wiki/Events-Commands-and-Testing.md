...
XLib also now ships a source-tracked damage modifier profile API for cases where you want reusable framework-side vulnerability, resistance, or immunity rules instead of wiring listeners yourself.

Useful APIs:

- `DamageModifierProfileApi.register(...)`
- `DamageModifierProfileApi.grant(player, profileId, sourceId)`
- `DamageModifierProfileApi.revoke(player, profileId, sourceId)`
- `DamageModifierProfileApi.syncSourceProfiles(player, sourceId, profileIds)`

Profiles can match exact damage type ids and damage-type tags for both incoming and outgoing damage. Matching multipliers stack multiplicatively, so `0.0` is immunity, values between `0.0` and `1.0` reduce damage, and values above `1.0` increase damage.
...
- `/xlib classification grant_entity_type|revoke_entity_type|grant_tag|revoke_tag|clear_source|clear_all|list ...`
...
- `/xlib capability_policy ...`, `/xlib classification ...`, `/xlib bindings ...`, `/xlib stages ...`, `/xlib visual_form ...`, and `/xlib body ...` expose the newer entity/form runtime systems directly instead of forcing attachment edits or custom debug code.
...
- `/xlib debug state` now also reports detector windows, active/unlocked artifacts, identities, grant bundles, selected profiles, pending profile groups, synthetic classifications, active food replacement, and per-value tracked-value state; `/xlib debug source` includes structured ownership descriptors plus grouped identities/bundles/artifact unlocks by source id; and `/xlib debug export` now includes `detector_states`, `artifact_states`, `artifact_unlock_sources`, `identity_states`, `grant_bundle_states`, `grant_bundle_sources`, `selected_profiles`, `pending_profile_groups`, `profile_selection_data`, `managed_unlock_sources`, `source_descriptors`, synthetic classification sections, and the earlier passive/mode/state sections in the JSON snapshot.
