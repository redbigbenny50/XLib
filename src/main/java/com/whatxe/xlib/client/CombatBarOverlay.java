...
public final class CombatBarOverlay {
    ...
    private static void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        boolean showCombatHud = AbilityClientState.isCombatBarActive(minecraft);
        Optional<ResourceLocation> activeFoodReplacementValueId = TrackedValueApi.activeFoodReplacementValueId(minecraft.player);
        if (!showCombatHud && activeFoodReplacementValueId.isEmpty()) {
            return;
        }

        AbilityData data = ModAttachments.get(minecraft.player);
        CombatHudPresentation presentation = currentHudPresentation();
        if (activeFoodReplacementValueId.isPresent()) {
            TrackedValueDefinition definition = TrackedValueApi.findDefinition(activeFoodReplacementValueId.get()).orElse(null);
            if (definition != null) {
                renderFoodReplacementBar(
                        guiGraphics,
                        minecraft,
                        presentation,
                        definition,
                        TrackedValueApi.value(minecraft.player, definition.id())
                );
            }
        }
    ...
    private static void renderFoodReplacementBar(
            GuiGraphics guiGraphics,
            Minecraft minecraft,
            CombatHudPresentation presentation,
        ...
        int width = 81;
        int height = 10;
        double clampedAmount = definition.clamp(currentAmount);

        Optional<TrackedValueHudRenderer> customRenderer = TrackedValueHudApi.findRenderer(definition.id());
        if (customRenderer.isPresent()) {
            int currentValue = (int) Math.round(clampedAmount);
            int maxValue = (int) Math.round(definition.maxValue());
            customRenderer.get().render(
                    guiGraphics,
                    minecraft,
                    minecraft.player,
                    definition,
        ...
        double range = Math.max(1.0E-9D, definition.maxValue() - definition.minValue());
        double normalized = Math.max(0.0D, Math.min(1.0D, (clampedAmount - definition.minValue()) / range));
        int fillWidth = (int) Math.round((width - 2) * normalized);
        String label = fitLabel(minecraft, definition.displayName().getString(), Math.max(24, width / 2));
        String amountLabel = formatTrackedValueAmount(clampedAmount) + "/" + formatTrackedValueAmount(definition.maxValue());

        guiGraphics.fill(x, y, x + width, y + height, presentation.resourceOuterFrameColor());
        guiGraphics.fill(x + 1, y + 1, x + width - 1, y + height - 1, presentation.resourceInnerFrameColor());
        guiGraphics.fill(x + 1, y + 1, x + width - 1, y + height - 1, presentation.resourceBackgroundColor());
        if (fillWidth > 0) {
            guiGraphics.fill(
                    x + 1,
                    y + 1,
                    x + 1 + fillWidth,
                    y + height - 1,
                    CombatBarPreferences.mapHudColor(definition.hudColor())
            );
        }
        ...
        guiGraphics.drawString(
                minecraft.font,
                amountLabel,
                x + width - minecraft.font.width(amountLabel),
                y - 9,
                presentation.resourceValueColor(),
                false
        );
    ...
    private static String formatTrackedValueAmount(double amount) {
        long roundedValue = Math.round(amount);
        if (Math.abs(amount - roundedValue) < 0.001D) {
